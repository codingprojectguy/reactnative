...
import{ createTable, getMenuItems, saveMenuItems, filterByQueryAndCategoories } from "../database";
...
(async ()=> {
  ...
  try{
    ...
    const getProfile = await AsyncStorage.getIem("profile");
  } catch(e){
...
useUpdateEffect(()=> {
  (async ()=>{
    const activeCategories = sections.filter((s,i)=>{
      if(filterSections.every(item => item === false)){
        return true;
      }
      return filterSections[i];
    });
    try {
      const menuItems = await filterByQueryAndCategoories(
        query,
        activeCategories
      );
      const sectionListData =getSectionListData(menuItems);
